...
def plot_accidents_by_year(ax):
    ...
    df['Year'].value_counts().sort_index().plot(ax=ax)
...
def plot_accidents_by_state(ax):
    ...
    ax.set_title("Top 10 stanów wg liczby wypadków")
    ax.set_xlabel("Stan")
    ax.set_ylabel("Liczba wypadków")
...
def plot_accidents_by_severity(ax):
    ...
    ax.set_xlabel("Skala wypadku (1 = najmniej poważny, 4 = najpoważniejszy)")
    ax.set_ylabel("Liczba wypadków")
...
def plot_accidents_by_weather(ax):
    ax.clear()
    weather = df['Weather_Condition'].fillna("Unknown")
    weather.value_counts().head(10).plot(kind='bar', ax=ax)
    ax.set_title("Top 10 warunków pogodowych przy wypadkach")
...
for i, name in enumerate(plot_names):
    tk.Button(btn_frame, text=name, command=lambda i=i: show_plot(i)).pack(side=tk.LEFT, padx=5, pady=5)
